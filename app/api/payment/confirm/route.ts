...
export async function POST(req: NextRequest) {
  try {
    ...
    const isRetake = checkoutSession.metadata?.isRetake === "true";
    ...
    if (isRetake) {
      await admin
        ...
        .eq("id", session.user.id);
      console.log(
        `[payment/confirm] Retake – reset onboarded for user ${session.user.id}`,
      );
    }

    console.log(`[payment/confirm] ✓ has_paid set for user ${session.user.id}`);

    // Refresh the session to pick up changes
