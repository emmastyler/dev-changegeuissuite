...
export default function AssessmentTakePage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        setCurrentIndex(Math.min(data.lastQuestionIndex, TOTAL_QUESTIONS - 1))
  ...
  const saveAnswer = useCallback(async (qId: string, value: number, qIndex: number) => {
    ...
    try {
      const res = await fetch('/api/assessment/answer', {
        ...
        body: JSON.stringify({ assessmentId, questionId: qId, value, questionIndex: qIndex }),
      })
  ...
  // ── Navigation ───────────────────────────────────────────────
  function goNext() {
    if (currentAnswer == null) return // must select before proceeding
  ...
  async function handleSubmit() {
    ...
    if (!allAnswered) {
      ...
      setError(`Please answer all questions. ${TOTAL_QUESTIONS - answered} remaining.`)
      return
    }
  ...
  // ── Loading states ───────────────────────────────────────────
  if (authLoading || loadingState === 'init') return (
    <div style={{ minHeight: '100vh', background: 'var(--sage)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 10 }}>Loading your assessment…</div>
        <div style={{ width: 200, height: 3, background: 'var(--border)', borderRadius: 2, overflow: 'hidden', margin: '0 auto' }}>
          <div style={{ height: '100%', background: 'var(--blue)', width: '40%', animation: 'loadbar 1.2s ease infinite', borderRadius: 2 }} />
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--sage)', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Progress bar */}
      <div style={{ height: 4, background: 'rgba(10,37,64,.08)', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 20px' }}>
        <div style={{ width: '100%', maxWidth: 640 }}>
          ...
          {/* Question card */}
          <div style={{ background: 'white', borderRadius: 'var(--radius)', padding: '40px 44px', boxShadow: '0 2px 16px rgba(10,37,64,.07)', marginBottom: 16 }}>
            ...
            {/* Question text */}
            <h2 style={{ fontSize: 'clamp(17px,2.5vw,22px)', fontWeight: 700, color: 'var(--navy)', lineHeight: 1.5, marginBottom: 32, letterSpacing: '-0.2px' }}>
              {question.text}
            </h2>

            {/* ── RADIO-STYLE ANSWER OPTIONS ── */}
            <fieldset style={{ border: 'none', padding: 0, margin: 0 }}>
              <legend style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-3)', marginBottom: 14 }}>
                Select one answer:
              </legend>
                  ...
                  return (
                    <label key={val} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
                        <span style={{ fontSize: 13, fontWeight: 700, color: selected ? 'var(--blue)' : 'var(--text-4)', minWidth: 16 }}>{val}</span>
                        <span style={{ fontSize: 14, fontWeight: selected ? 600 : 400, color: selected ? 'var(--navy)' : 'var(--text-2)' }}>
                          {LABELS[val - 1]}
                        </span>
                      </div>

                      {selected && <span style={{ fontSize: 16, color: 'var(--blue)', marginLeft: 'auto' }}>✓</span>}
                    </label>
                  )
                })}
              </div>
          ...
          {/* Error */}
          {error && (
            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '10px 16px', fontSize: 13, color: '#dc2626', marginBottom: 12 }}>
              {error}
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
            ...
            <button onClick={goBack} disabled={currentIndex === 0} style={{
              ...
              opacity: currentIndex === 0 ? 0.5 : 1, fontFamily: 'Inter,sans-serif',
              transition: 'all .15s',
            }}>
              ← Back
            </button>
            ...
            <div style={{ display: 'flex', gap: 5, alignItems: 'center', flex: 1, justifyContent: 'center' }}>
              {Array.from({ length: 9 }, (_, i) => {
                ...
                return (
                  <button key={i} onClick={() => setCurrentIndex(qi)} title={`Question ${qi + 1}`} style={{
            ...
            {isLast ? (
              <button onClick={handleSubmit} disabled={!allAnswered} style={{
                padding: '11px 28px', borderRadius: '100px', border: 'none',
              ...
              <button onClick={goNext} disabled={currentAnswer == null} style={{
                padding: '11px 28px', borderRadius: '100px', border: 'none',
