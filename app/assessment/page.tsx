...
export default function AssessmentPage() {
  ...
  useEffect(() => {
    if (!loading && !isAuthenticated) router.push('/login?returnUrl=/assessment')
  }, [loading, isAuthenticated, router])

  if (loading) return <div style={{ minHeight:'100vh',background:'var(--sage)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,color:'var(--text-3)' }}>Loading…</div>
    ...
    <div style={{ minHeight:'100vh', background:'var(--sage)' }}>
      ...
      <div className="page">
        <div className="card" style={{ padding:52 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'var(--blue)', textTransform:'uppercase', letterSpacing:'1.5px', marginBottom:12 }}>Change Genius™ Assessment</div>
          <h1 style={{ fontSize:36, fontWeight:800, color:'var(--text-1)', letterSpacing:'-1px', marginBottom:14, lineHeight:1.15 }}>Discover Your Change Genius™ Role</h1>
          <p style={{ fontSize:16, color:'var(--text-2)', lineHeight:1.7, marginBottom:32, maxWidth:560 }}>60 behavioural questions. 8–10 minutes. No right or wrong answers — you are always the expert about your own behaviour during change.</p>
          ...
          <p style={{ fontSize:12, color:'var(--text-4)', marginTop:14 }}>One-time payment of $24 · No subscription required</p>
        </div>
      </div>
    </div>
  )
